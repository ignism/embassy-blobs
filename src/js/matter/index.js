...
class MatterApp {
  constructor(wrapper, embassies, patterns, numBlobs, debug = false) {
    ...
    this.dish = new Dish(this.dishOrigin, 96, this.dishSize)
    this.dishOuter = new Dish(this.dishOrigin, 96, this.dishSize + 4)
    this.numBlobs = numBlobs
    this.rNorm = this.dishSize * 0.525
    this.blobRadi = [this.rNorm * .95, this.rNorm * 0.8, this.rNorm * 0.75, this.rNorm * 0.5]
    this.blobScales = [this.rNorm * .95 / 24, this.rNorm * 0.8 / 24, this.rNorm * 0.75 / 24, this.rNorm * 0.5 / 24]
    this.highlightScales = [this.rNorm * 1.2 / 24, this.rNorm * 0.7 / 24, this.rNorm * 0.65 / 24, this.rNorm * 0.45 / 24]
    this.blobs = []
    this.overblob = -1
    this.svgRenders = []
    this.debug = debug
